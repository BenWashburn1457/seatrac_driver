...
class MyDriver : public SeatracDriver
{
    ...
    MyDriver(const std::string& serialPort, std::ostream* ostream_ptr) :
        SeatracDriver(serialPort)
    { this->ostream_ptr = ostream_ptr; }
            ...
            case CID_DAT_RECEIVE:
                {
                    ...
                    // time, source beacon id, pitch, roll, yaw, rssi, azimuth, elevation, custom_data
                    *ostream_ptr   << ctime(&my_time) << ", "
                                    << response.acoFix.srcId << ", "
                                    << std::to_string(response.acoFix.attitudePitch) << ", "
                                    << std::to_string(response.acoFix.attitudeRoll) << ", "
                                    << std::to_string(response.acoFix.attitudeYaw) << ", "
                                    << std::to_string(response.acoFix.rssi) << ", "
                                    << std::to_string(response.acoFix.usbl.azimuth) << ", "
                                    << std::to_string(response.acoFix.usbl.elevation) << ", "
                                    << response.packetData //<< ", "
                                    << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    std::ofstream write_stream(argv[2]); //arg 2 is the file name

    write_stream << "time, src beacon id, pitch, roll, yaw, rssi, azimuth, elevation, custom_data\n";

    MyDriver seatrac(serial_port, &write_stream);
    ...
    for (int i=0; i<50; i++) {
        ...
        seatrac.send_data(
            BEACON_10,
            MSG_REQU, //other valid types: MSG_OWAY, MSG_REQ, MSG_REQU, MSG_REQX
            (uint8_t)message_txt.size(),
            (const uint8_t*)message_txt.data()
          );
    }
